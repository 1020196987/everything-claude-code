...
description zh-CN: Ktor 服务器模式，包括路由 DSL、插件、认证、Koin DI、kotlinx.serialization、WebSockets 和 testApplication 测试。
origin: ECC
---
...
使用 Kotlin 协程构建健壮、可维护 HTTP 服务器的综合 Ktor 模式。
...
**记住**：Ktor 围绕 Kotlin 协程和 DSL 设计。保持路由精简，将逻辑推送到服务，使用 Koin 进行依赖注入。使用 `testApplication` 进行全面集成测试。
