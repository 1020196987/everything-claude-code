...
description zh-CN: JetBrains Exposed ORM 模式，包括 DSL 查询、DAO 模式、事务、HikariCP 连接池、Flyway 迁移和仓库模式。
origin: ECC
---
...
使用 JetBrains Exposed ORM 进行数据库访问的综合模式，包括 DSL 查询、DAO、事务和生产级配置。
...
Exposed 提供两种查询风格：DSL 用于直接的类 SQL 表达式，DAO 用于实体生命周期管理。HikariCP 通过 `HikariConfig` 管理可复用数据库连接池。Flyway 在启动时运行版本化 SQL 迁移脚本以保持模式同步。所有数据库操作都在 `newSuspendedTransaction` 块内运行，以确保协程安全性和原子性。仓库模式将 Exposed 查询包装在接口后面，使业务逻辑与数据层解耦，测试可以使用内存 H2 数据库。
...
// 正确：使用 newSuspendedTransaction 支持协程
suspend fun performDatabaseOperation(): Result<User> =
    runCatching {
        newSuspendedTransaction {
            val user = UserEntity.new {
...
| `object Table : UUIDTable("name")` | 使用 UUID 主键定义表 |
...
**记住**：简单查询使用 DSL 风格，需要实体生命周期管理时使用 DAO 风格。始终使用 `newSuspendedTransaction` 以支持协程，并将数据库操作包装在仓库接口后面以提高可测试性。
